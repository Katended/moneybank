...
if(move_uploaded_file($_FILES['file']['tmp_name'],DIR_WS_TEMP_FILES.'temfile.xls')==1 && $_POST['save']=="N"){
	...
	if (eregi('(xls|XLS)$',$_FILES['file']['tmp_name'])) {		
		...
		$save = 'N';
		$msg =" Invalid file format! Please use a Microsoft Excel file to load clients or contact Sync Software Ltd";	
	}
...
if($_POST['save']=="Y"){
	  ...
	  foreach($reader->sheets as $k=>$data){	  
		  ...
		  if ($data[numRows]!=0){
			  foreach($data['cells'] as $row){
				  ...
				  $feecategories_name = tep_db_prepare_input($row[1]);
				  $feecategories_description = tep_db_prepare_input($row[2]);
								 				  
				  $db_query=tep_db_query("SELECT feecategories_id FROM " . TABLE_FEE_CATEGORIES . " WHERE feecategories_name='".$feecategories_name."'");
				  
				  if(!tep_db_num_rows($db_query)){
					  if($i!=0){
						
						 tep_db_query("INSERT INTO " . TABLE_FEE_CATEGORIES . " (feecategories_datecreated,feecategories_dateupdated,feecategories_name,feecategories_dateeffective,feecategories_description) values (NOW(),NOW(),'" . tep_db_input($feecategories_name) . "',NOW(),'" . tep_db_input($feecategories_description) ."')");
	...
	if(file_exists(DIR_WS_TEMP_FILES.'temfile.xls')){
	 ...
	 $msg ="Fees Categories have been successfully loaded.";
...
<?php require('../'.DIR_WS_INCLUDES . 'initmenu.php'); ?>
 ...
 <form action="importfeescat.php?action=<?php echo $action;?>" method="post" enctype="multipart/form-data"  id="frmimportstudents" name="frmimportstudents">   
	...
	<table width="100%" border="0" cellspacing="2" cellpadding="2">
	  ...
	  <tr>
		<td align="right" valign="bottom">Select File:</td>
		<td><input name="file" type="file"  id="file"  size="80" width="100"></td>
	  </tr> 
	  <tr>
		<td colspan="2" align="center">
		<input name="submit" type="submit" value="Import" class="actbutton"><input name="submit" type="submit" value="Save" class="actbutton"><input name="Reset" type="reset" value="Reset" class="actbutton"></td>
	  </tr>
	</table>
